   ...
   if lista==[]:
    ...
    if lista==[]:
     ...
     while True:
        ...
        if numero not in listamatricole:
    ...
    if lista==[]:
     ...
     while True:
      matricolaa=input("inserisci matricola dell'alunno a cui vuoi visualizzare i compiti")
      if matricolaa not in listamatricole:
        print("Matricola alunno non esistente! 🚨")
      else:
       break
     ...
     for elementi in listacomp:
         ...
         for chiave_esterna, dizionario_interno in elementi.items():
           if lista_compiti[task]["alunno_matricola"]==matricolaa:
            print(f"--- {chiave_esterna} ---")
            for chiave_interna, valore_interno in dizionario_interno.items():
             print(f"{chiave_interna} : {valore_interno}")
     ...
     if lista==[]:
               ...
               for chiave_esterno, dizionario_interna in elementa.items():
                 if chiave_esterno==elementi:
                  ...
                  for chiave_interno, valore_interna in dizionario_interna.items():
                   ...
                   listadiz.append(valore_interna)
    
     
             sommadiz=sum(listadiz)
             lunghezzadiz=len(listadiz)
             mediadiz=sommadiz/lunghezzadiz
             diz[elementi]=mediadiz
       ...
       d_ordinate = dict(sorted(diz.items(),reverse=True))
       for chiaveesterna,chiaveinterna in d_ordinate.items():
          print(f"{chiaveesterna} : {chiaveinterna}")
