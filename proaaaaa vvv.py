   ...
   if lista==[]:
    ...
    if lista==[]:
     ...
     if lista==[]:
        ...
        while True:
         statistiche=input("scegli l'alunno a cui visualizzare le statistiche :(inserire matricola alunno) ")
         if statistiche not in listamatricole:
            print("Matricola alunno non esistente! 🚨")
         else:
            break
        ...
        for compito in listacomp:
          for task_id in compito:
           dati = compito[task_id]

        if dati["alunno_matricola"] == statistiche:
            somma_voti = somma_voti + dati["voto"]
            numero_voti = numero_voti + 1

        if numero_voti == 0:
            print("Questo studente non ha ancora voti.")
        else:
         media = somma_voti / numero_voti
         print("La media dello studente", statistiche, "è:", media)
